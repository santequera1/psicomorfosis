 ...
 *
 * Sin esto, entrar con Google solo funciona si el correo de la plataforma
 * es el mismo del Gmail. Al vincular guardamos el `sub` de Google (su id
 * estable), así que quien inicia sesión con una casilla propia
 * (p. ej. @psicomorfosis.co) también puede entrar con su Google personal.
 *
 * El flujo manda el JWT actual en `?link=`: es la prueba de que quien
 * conecta la cuenta es el dueño de la sesión.
 */
function GoogleLinkCard() {
  ...
  // Al volver del callback, /configuracion trae ?google_ok o ?google_error.
  useEffect(() => {
    const q = new URLSearchParams(window.location.search);
    const ok = q.get("google_ok");
  ...
  // El token viaja en la cabecera, no en la URL: pedimos la URL de Google
  // al backend y navegamos a ella.
  const link = useMutation({
    mutationFn: () => api.googleLinkStart(),
    onSuccess: ({ url }) => { window.location.href = url; },
    onError: (e: Error) => toast.error(e.message),
  });

  const unlink = useMutation({
    mutationFn: () => api.googleUnlink(),
    onSuccess: () => {
    ...
    <div className="rounded-xl border border-line-200 bg-surface p-5 mb-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex items-start gap-3">
          <svg viewBox="0 0 48 48" className="h-5 w-5 mt-0.5 shrink-0" aria-hidden>
          ...
          <div>
            <h3 className="text-sm font-medium text-ink-900">Entrar con Google</h3>
            {data.linked ? (
              <p className="text-xs text-ink-500 mt-1">
                Vinculada a <span className="text-ink-900 font-medium break-all">{data.email}</span>.
                Puedes entrar con Google o con tu usuario y contraseña, como prefieras.
              </p>
            ) : (
              <p className="text-xs text-ink-500 mt-1">
                Conecta tu cuenta de Google para entrar con un clic. Tu usuario y
                contraseña seguirán funcionando igual.
              </p>
            )}
          </div>
        </div>
        {data.linked ? (
          <button
            ...
            className="h-9 px-3 rounded-lg border border-line-200 text-ink-700 text-xs hover:border-rose-400 hover:text-rose-700 inline-flex items-center gap-1.5 disabled:opacity-50"
          >
            {unlink.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <X className="h-3.5 w-3.5" />}
            Desvincular
          </button>
        ) : (
          <button
            ...
            className="h-9 px-3 rounded-lg border border-line-200 text-ink-700 text-xs hover:border-brand-400 inline-flex items-center gap-1.5 disabled:opacity-50"
          >
            {link.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
            Vincular
          </button>
        )}
      </div>
      {data.linked && (
        <p className="text-[11px] text-ink-400 mt-3">
          Vinculada el {data.linkedAt ? new Date(data.linkedAt).toLocaleDateString("es-CO", { day: "numeric", month: "long", year: "numeric" }) : "—"}.
        </p>
      )}
    </div>
  );
