...
/**
 * Métrica anónima del perfil (visitas/clics). No cuenta al propio equipo
 * (sesión de staff en este navegador) y jamás rompe la página.
 */
function sendMetric(slug: string, type: string, extra?: { source?: string | null; network?: string }) {
  try {
    const u = getStoredUser();
    if (u && u.role !== "paciente") return;
    const body = JSON.stringify({ slug, type, ...(extra ?? {}) });
    if (navigator.sendBeacon) {
      navigator.sendBeacon("/api/public/profile-metric", new Blob([body], { type: "application/json" }));
    } else {
      fetch("/api/public/profile-metric", { method: "POST", headers: { "content-type": "application/json" }, body, keepalive: true }).catch(() => {});
    }
  } catch { /* nunca romper el perfil por una métrica */ }
}

/** Dispara la visita una vez montado el perfil (con la fuente de llegada). */
function VisitBeacon({ slug }: { slug: string }) {
  useEffect(() => {
    const utm = new URLSearchParams(window.location.search).get("utm_source");
    let ref: string | null = null;
    try { ref = document.referrer ? new URL(document.referrer).hostname.replace(/^www\./, "") : null; } catch { ref = null; }
    if (ref && ref.endsWith("psicomorfosis.co")) ref = null;
    sendMetric(slug, "visit", { source: utm || ref });
  }, [slug]);
  return null;
}
  ...
  return (
    <div className="min-h-[100svh] bg-bg-50 text-ink-900 relative overflow-x-clip">
      ...
      <main className="max-w-md mx-auto px-5 pt-12 pb-10 flex flex-col items-center text-center">
        ...
        >
          {wa && (
            <a href={wa} onClick={() => sendMetric(profile.slug, "click_whatsapp")} target="_blank" rel="noreferrer" aria-label="WhatsApp"
          ...
          {([
            ...
            return (
              <a key={key} href={href} target="_blank" rel="noreferrer noopener" aria-label={label}
                 onClick={() => sendMetric(profile.slug, "click_social", { network: key })}
          ...
          >
            {profile.links.map((l) => (
              <a
                ...
                onClick={() => sendMetric(profile.slug, "click_link")}
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={() => { sendMetric(profile.slug, "click_agendar"); setWizardOpen(true); }}
